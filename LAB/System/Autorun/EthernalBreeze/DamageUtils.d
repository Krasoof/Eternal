...
func int StExt_StunTarget(var c_npc target, var c_npc atk, var int chance)
{
	...
	// POSTURE (poise): Zakon bosses can only be stunned inside the ~3s window
	// after the player's PERFECT PARRY - and then reliably (chance boosted).
	// Outside the window they are stun-immune. Fight loop: parry -> riposte
	// -> posture window -> stun -> free hits.
	if ((target.id >= 99710) && (target.id <= 99725))
	{
		if (StExt_ZakonPosture_Window <= 0) { return false; };
		chance = chance + 35;
	};
